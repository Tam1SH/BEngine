...
namespace BEbraEngine {
    ...
    VkInstance BaseVulkanRender::instance;
    ...
    void BaseVulkanRender::allocateObjectTransformsSet()
    {
        ...
        vkAllocateDescriptorSets(GetDevice(), &allocInfo, &objectTransforms);

    }

    void BaseVulkanRender::_recreateCameraSet()
    {
    ...
    void BaseVulkanRender::CreateCameraSet(RenderBuffer* buffer)
    {
    ...
    VkCommandPool BaseVulkanRender::CreateCommandPool()
    {
        ...
        if (vkCreateCommandPool(BaseVulkanRender::device, &poolInfo, nullptr, &pool) != VK_SUCCESS) {
    ...
    VkCommandBuffer BaseVulkanRender::createCmdBuffer() {
    ...
    VkCommandBuffer BaseVulkanRender::createCmdBuffer(VkCommandPool pool)
    {
        VkCommandBuffer buffer;
        VkCommandBufferAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
        ...
        vkAllocateCommandBuffers(BaseVulkanRender::device, &allocInfo, &buffer);

        return buffer;
    }

    void BaseVulkanRender::createCopyCmdBuffer() {
        VkCommandBufferAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
        allocInfo.commandPool = commandPool;
        allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
    ...
    void BaseVulkanRender::createVkImage2(stbi_uc* pixels, int texWidth, int texHeight, VkImage& textureImage, VkDeviceMemory& textureImageMemory, VkCommandBuffer& buffer)
    {
    ...
    void BaseVulkanRender::createVkImage(stbi_uc* pixels, int texWidth, int texHeight, VkImage& textureImage, VkDeviceMemory& textureImageMemory, VkDeviceSize imageSize, VkBuffer& stagingBuffer)
    {
        ...
        vkEndCommandBuffer(_copyBuffer);
    ...
    void BaseVulkanRender::AddBufferToQueue(VkCommandBuffer buffer)
    {
        //BufferQueue.push(buffer);
    }

    VkFence* BaseVulkanRender::GetCurrentFence()
    {
    ...
    void BaseVulkanRender::createDepthResources()
    {
    ...
    VkFormat BaseVulkanRender::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
    {
    ...
    VkFormat BaseVulkanRender::findDepthFormat()
    {
    ...
    bool BaseVulkanRender::hasStencilComponent(VkFormat format)
    {
        return format == VK_FORMAT_D32_SFLOAT_S8_UINT || format == VK_FORMAT_D24_UNORM_S8_UINT;
    }


    void BaseVulkanRender::ResizeDescriptorPool(unsigned int count)
    {
    ...
    void BaseVulkanRender::cleanupSwapChain()
    {
    ...
    void BaseVulkanRender::cleanUpDefault()
    {
        cleanupSwapChain();

        vkDestroyDescriptorSetLayout(device, ObjectLayout, nullptr);
        vkDestroyDescriptorSetLayout(device, ObjectWithoutTextureLayout, nullptr);
        vkDestroyDescriptorSetLayout(device, CameraLayout, nullptr);
    ...
    void BaseVulkanRender::_createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
    {
    ...
    size_t BaseVulkanRender::pad_uniform_buffer_size(size_t originalSize)
    {
    ...
    void BaseVulkanRender::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
    {
        VkCommandBufferAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
        allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
    ...
    void BaseVulkanRender::copyBuffer1(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size, VkCommandBuffer& cmdBuffer)
    {
        VkBufferCopy copyRegion{};
        copyRegion.size = size;
    ...
    VkImageView BaseVulkanRender::createTextureImageView(VkImage& textureImage)
    {
        return createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
    }

    VkImage BaseVulkanRender::createTextureImage(const std::string& path, VkDeviceMemory& textureImageMemory)
    {
    ...
    VkImageView BaseVulkanRender::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags)
    {
    ...
    void BaseVulkanRender::createTextureSampler(VkSampler& textureSampler)
    {
    ...
    void BaseVulkanRender::populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo)
    {
        createInfo = {};
    ...
    void BaseVulkanRender::setupDebugMessenger()
    {
    ...
    void BaseVulkanRender::pickPhysicalDevice()
    {
    ...
    BaseVulkanRender::QueueFamilyIndices BaseVulkanRender::findQueueFamilies(VkPhysicalDevice device)
    {
        /*
    ...
    void BaseVulkanRender::createLogicalDevice()
    {
    ...
    void BaseVulkanRender::createVkImage1()
    {
        VkSubmitInfo submitInfo{};
        submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
        submitInfo.commandBufferCount = 1;
    ...
    void BaseVulkanRender::createImageViews()
    {
    ...
    void BaseVulkanRender::createRenderPass()
    {
    ...
    void BaseVulkanRender::createLineGraphicsPipeline() {
        ...
        std::array<VkDescriptorSetLayout, 2> layouts = { ObjectLayout, CameraLayout };
    ...
    void BaseVulkanRender::createGraphicsPipeline()
    {
        auto vertShaderCode = readFile("vert.spv");
        auto fragShaderCode = readFile("frag.spv");
    ...
    void BaseVulkanRender::createFramebuffers()
    {
    ...
    void BaseVulkanRender::createCommandPool()
    {
    ...
    void BaseVulkanRender::createDescriptorPool()
    {
    ...
    void BaseVulkanRender::createObjectDescriptorSetLayout()
    {
        ...
        if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &ObjectLayout) != VK_SUCCESS) {
    ...
    void BaseVulkanRender::createCameraDescriptorSetLayout()
    {
        ...
        if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &CameraLayout) != VK_SUCCESS) {
            throw std::runtime_error("failed to create descriptor set layout!");
        }
    }

    void BaseVulkanRender::createSyncObjects()
    {
    ...
    void BaseVulkanRender::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
    {
    ...
    void BaseVulkanRender::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, VkCommandBuffer& buffer)
    {
    ...
    void BaseVulkanRender::createCmdBuffers()
    {
    ...
    void BaseVulkanRender::UpdateFrame()
    {
    ...
    void BaseVulkanRender::UpdateCmdBuffers()
    {
        OnDrawFrame();

    }

    VkShaderModule BaseVulkanRender::createShaderModule(const std::vector<char>& code)
    {
    ...
    uint32_t BaseVulkanRender::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
    {
        VkPhysicalDeviceMemoryProperties memProperties;
        vkGetPhysicalDeviceMemoryProperties(BaseVulkanRender::physicalDevice, &memProperties);
    ...
    void BaseVulkanRender::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height, VkCommandBuffer& cmdbuffer)
    {
    ...
    VkSurfaceFormatKHR BaseVulkanRender::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats)
    {
    ...
    VkPresentModeKHR BaseVulkanRender::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes)
    {
    ...
    VkExtent2D BaseVulkanRender::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities)
    {
    ...
    BaseVulkanRender::SwapChainSupportDetails BaseVulkanRender::querySwapChainSupport(VkPhysicalDevice device)
    {
    ...
    bool BaseVulkanRender::isDeviceSuitable(VkPhysicalDevice device)
    {
    ...
    bool BaseVulkanRender::checkDeviceExtensionSupport(VkPhysicalDevice device)
    {
    ...
    std::vector<const char*> BaseVulkanRender::getRequiredExtensions()
    {
    ...
    bool BaseVulkanRender::checkValidationLayerSupport()
    {
        uint32_t layerCount;
    ...
    std::vector<char> BaseVulkanRender::readFile(const std::string& filename)
    {
    ...
    VkResult BaseVulkanRender::CreateDebugUtilsMessengerEXT(VkInstance instance, const VkDebugUtilsMessengerCreateInfoEXT* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDebugUtilsMessengerEXT* pDebugMessenger)
    {
    ...
    void BaseVulkanRender::DestroyDebugUtilsMessengerEXT(VkInstance instance, VkDebugUtilsMessengerEXT debugMessenger, const VkAllocationCallbacks* pAllocator)
    {
    ...
    {
        auto app = reinterpret_cast<BaseVulkanRender*>(glfwGetWindowUserPointer(window));
        app->recreateSwapChain();
        app->UpdateFrame();
    }
    */
    VkDevice BaseVulkanRender::GetDevice()
    {
        return device;
    }
    ...
    bool BaseVulkanRender::IsCreate()
    {
        return isCreate;
    }
}